...
2. attributi `data-i` e `data-d` per il cambio lingua;
3. eventuali valori duplicati in card servizi, tabella prezzi, form e QR/mailto.

## Modificare testi bilingue

Ogni testo traducibile segue questo schema:

```html
<p data-i="Testo italiano" data-d="Testo tedesco">Testo tedesco</p>
```
...
1. aggiornare `data-i`;
2. aggiornare `data-d`;
3. aggiornare anche il contenuto interno dell'elemento con il testo tedesco, perché la lingua iniziale è DE.

Se il contenuto contiene HTML, come `<em>` o `<strong>`, va incluso anche dentro gli attributi `data-i`/`data-d`.
...
6. verificare entrambi gli attributi `data-i` e `data-d`;
